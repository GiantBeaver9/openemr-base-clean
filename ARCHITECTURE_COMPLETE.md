...
| I15 | **Cleaning fidelity is audited.** Pre- and post-clean snapshots are captured for every extraction; an independent LLM auditor (Flash) confirms each cleaned fact faithfully represents its raw source (value, unit, status, no material drop). A failure re-runs the extract→audit cycle up to 3× on the same source; persistent failure quarantines the entity as a visible `suspect` fact and alerts — never a silent content error, never an infinite loop. Deepens I14 (entity survival) to content fidelity. (T23) | clean-audit layer (§2.6) + `mod_copilot_clean_audit` |
...
mod_copilot_clean_audit:                     -- append-only; cleaning-fidelity provenance (T23, I15)
  id (pk) · correlation_id (indexed) · capability · entity_key
  raw_snapshot (JSON: source rows/resources as read)
  cleaned_snapshot (JSON: typed facts produced)
  verdict (pass | fail | quarantined) · findings (JSON, per-entity)
  attempt (1..3) · model · tokens_in · tokens_out · cost_usd · created_at
```
...
|---|---|---|---|---|
| U1 Module skeleton | composer.json, info.txt, openemr.bootstrap.php, `src/Bootstrap.php` (event subscription, menu/page registration, ACL gate), `table.sql` (all six `mod_copilot_*` tables + background_services row), ModuleManagerListener | module root files, `src/Bootstrap.php` | — | installs/enables/disables/uninstalls cleanly; additivity tests 2–3 pass (I9) |
...
| U12 Observability | Correlation-ID middleware; `mod_copilot_trace` writer on every path (I12); dashboard page (requests, error rate, p50/p95, per-tool failures, retries, verification pass rate, cache hit rate, cost) with click-through to span waterfall + payloads; alert evaluator on worker tick (9 alerts, ARCHITECTURE.md §3.5; heartbeat, I14 unmapped-entity, I15 clean-audit alerts); mapping-conservation counters (I14) on extract/tool_call spans; rate-limit/circuit-breaker enforcement (§3.7); `/copilot/health` + `/copilot/ready` (module-standalone; no host prober registration — §3.4) | `src/Observability/`, `templates/dashboard*` | U1 (consumed by all) | every eval run leaves reconstructable traces (spot-check: answer the 4 case-study questions from the table alone); cache-hit and degraded paths produce spans; /ready fails when LLM stub down while /health stays green |
...
| U14 Cleaning-accuracy audit | Pre/post snapshot capture; Flash auditor client + strict pass/fail schema; extract→audit retry (≤3, I15); quarantine-on-persistent-fail (visible `suspect` fact + I15 alert); runs on the warm path; `mod_copilot_clean_audit` writer | `src/CleanAudit/`, `mod_copilot_clean_audit` | U4, U7, U9 | seeded mis-map / corrupted-value / dropped-sub-field fixtures are flagged; a transient/flaky fail clears on retry; 3× persistent fail quarantines (never loops, never silently passes); audit verdict + tokens + cost land on the trace; runs warm ⇒ no read-path latency added |
...
3. **BAA-covered hosting for real PHI (T24).** The app is hosted on Railway for cost (T24), which does not offer a HIPAA BAA. Synthetic-only this phase (OPEN-1) makes that acceptable; a real-PHI deployment requires either a BAA-covered Railway offering or migration to a BAA-covered host (cloud VM/k8s). The Vertex BAA for the *LLM* is independent of where the app runs and is unaffected.
