...
## LLM platform (decided): Google Gemini on Vertex AI (T18) — app hosted on Railway (T24)
...
- **Hosting (decided, T24):** the app (OpenEMR + module + MySQL) runs on **Railway** — a low-cost container PaaS — chosen over GCP Compute Engine on budget; the LLM stays on Vertex, reached over HTTPS with a GCP service account (two vendors, one for compute, one for the LLM). Scaling is **vertical-first** — resize the Railway service's CPU/RAM, not horizontal app replicas (§3.6, T20). **BAA note:** Railway is not a HIPAA-BAA host for PHI, which is fine this phase (synthetic patients only, OPEN-1); a real-PHI deployment needs a BAA-covered host (OPEN-3) — the Vertex BAA for the LLM is independent of where the app runs.
...
| V4 | **Numeric grounding** — every number in claim text (values, dates, counts) must appear in a cited fact, after canonicalization (units, ISO dates, decimal normalization). Derived numbers — deltas, counts, spans, expected-return dates — are legal **only** as citations of `derived_*` facts computed deterministically by capabilities (fact schema): the verifier never does arithmetic, and neither does the model | the classic hallucination: right citation, wrong number; transposed A1c values; invented dates; LLM-invented arithmetic ("rose 0.6" with no derived fact behind it) |
...
- **Over-reliance is measured, not assumed away:** citation click-through rate and facts-panel opens are dashboard metrics (§3.3). If they decay over weeks, that is the leading indicator that the residual risks above are landing on a physician who has stopped looking — and it pages product, not on-call. These are real limits of deterministic verification. They are bounded, not solved, by: facts-first rendering (the full fact table is always on the same page — prose is never the only view), the eval suite's narrative checks (known-answer fixtures where emphasis is asserted), and the **second-pass reviewer** (bullet 2 above) — the same Flash pass, surfaced to the reader and feeding evals/dashboards, deliberately **never the blocking gate** (a second model is a second hallucination surface — T15).

### 2.6 Cleaning-accuracy audit — pre/post fidelity of the deterministic extraction

I14 proves no entity was *dropped*; it does not prove each surviving fact *faithfully represents* its raw source. A mapper can keep an entity but mis-read it — pick the wrong field (`valueString` where the value was in `valueQuantity`), drop a sub-field (an abnormal flag, a comment), or coerce a value — and the entity count still balances. This layer guards **content fidelity** of the cleaning, one level deeper than I14's survival check.

- **Pre/post capture.** Every extraction records two snapshots to `mod_copilot_clean_audit` (append-only, module-owned, PHI-stays-in-EMR per T16): the **raw** source rows/resources as read, and the **cleaned** typed facts produced from them — keyed by correlation_id + capability + entity.
- **The accuracy check (LLM auditor).** An independent model (Gemini Flash, T18) is handed the paired raw + cleaned snapshot and answers one strict-schema question per entity: *does the cleaned fact faithfully and completely represent the raw source — same value, same unit, same status, nothing material dropped?* It returns a structured pass/fail + per-entity findings, never free prose. This is an **auditor, not an extractor** — it produces no facts and touches no chart, exactly the role the §2 verifier plays over narration; **T1 holds** (the LLM still never extracts, it only checks the deterministic PHP that does).
- **Retry, up to 3× off the same data (I15).** A failing entity's extract→audit cycle re-runs up to 3 times (versioned config, same bound as the synthesis retry). Re-running the *same source* is deliberate: it clears the two recoverable causes — a transient partial read, and a flaky auditor verdict (Flash nondeterminism) — without changing the data. It is not expected to alter a correct-but-flagged result.
- **Persistent failure escalates — never loops, never passes silently.** Still failing after 3 attempts ⇒ the entity is **quarantined**: surfaced as a visible `suspect` fact (cited, counted like an I14 `unmapped`), held out of the narrative's trusted set, the clean-audit alert fires (§3.5), and the raw/cleaned pair becomes a U14 fixture. The rest of the extraction proceeds — degrade visibly, never blank the synthesis (§6/I6 stance).
- **Where it runs (cost, faced honestly).** On the **warm worker path**, off the physician's read latency; on **Flash** (cheapest tier); at configurable cadence (every extraction by default, sampling when cost-tuning); bounded by the §3.7 spend caps. It adds LLM spend proportional to extraction volume — the deliberate price of clinical-grade cleaning fidelity, modeled per-tier in the AI cost analysis. Verdict, attempts, tokens, and cost land on the trace and dashboard (§3.3).

**Layered defense, stated plainly:** **I14** = did every entity survive? **§2.6/I15** = does each cleaned fact faithfully represent its raw source? **§2 (V1–V6)** = does the narration faithfully represent the facts? Three independent guards at three boundaries — a silent data error has to beat all three.
...
· entities_in · facts_out · excluded_n · unmapped_n   (extract/tool_call spans — I14 conservation)
...
- **Mapping conservation (I14) — the over-stripping guard.** The fact layer strips aggressively (source rows/resources → only the typed facts it models), and that aggression introduces a *silent* failure: an entity dropped because the mapper didn't recognize its shape — a new `result_status`, a value in an unexpected field (the classic `valueString` where we expected `valueQuantity`), a source row the query assumptions missed. So `extract`/`tool_call` spans carry, per capability and code-set, a **conservation balance**: `entities_in = facts_out + excluded_n + unmapped_n`. `excluded_n` is the I5 rule-based accounting; **`unmapped_n`** is the new signal — entities the mapper could not place at all. A nonzero `unmapped_n` means our PHP dropped something the source contained (15 observations silently became 14 facts): it surfaces as a visible "N unmapped" fact on the doc, a dashboard metric (§3.3), and an alert (§3.5). This is the failure I5 alone can't catch — I5 accounts for rows the contract *chose* to exclude; conservation catches rows the mapper *silently missed*.
...
| **Unmapped-entity rate** (I14) | `unmapped_n` > 0 sustained, or any per-capability spike | our PHP stripped an entity the source contained — field/schema drift, or a source shape the mapper doesn't recognize (silent data loss, no exception) | diff `entities_in` vs `facts_out + excluded_n` on the flagged spans; the payload shows the unrecognized entity; fix the mapping and add the shape to U2 fixtures before closing |
| **Cleaning-audit fail rate** (I15) | quarantines > 0, or fail rate > 2% per capability over 1 h | the cleaner is mis-reading a *surviving* entity — wrong field, dropped sub-field, coerced value (content error, not a drop) | open the `mod_copilot_clean_audit` row: raw-vs-cleaned snapshot shows the discrepancy; fix the mapper, add the raw/cleaned pair to U14 fixtures |
...
**Scaling stance (decided, T20): vertical-first.** The app does not scale horizontally — it scales by resizing the service. OpenEMR is a session-holding monolith and the module is in-process by design (T2); each open chat connection holds a PHP worker for the turn's duration, so concurrency is bought with worker count, which is bought with CPU and RAM on a bigger **Railway service** (T24; the same stance applied to a GCE machine before the move) — not with app replicas (which would immediately raise shared-session, worker-singleton, and sticky-connection problems the monolith isn't built for). The load-test saturation numbers tell us the worker/RAM sizing per N users; the honest ceiling is stated rather than hidden: that ceiling arrives sooner on Railway's resource caps than on a large cloud VM (the cost T24 accepts), and beyond it (multi-clinic scale) the moves are — migrate to a higher-ceiling host, then read replicas for fact extraction, then pulling the chat loop out of process — a deliberate re-opening of T2/T24, not an emergency.
...
- **Trust boundaries:** (1) browser ↔ module endpoint — host session + CSRF; (2) module ↔ clinical tables — read-only, through host services, every read audit-logged via `EventAuditLogger`; (3) module ↔ LLM provider — the only place data leaves the process; BAA assumed per the case-study ground rules, **synthetic patients only** in this phase (OPEN-1 in the complete spec), and a named redaction/BAA review is a hard gate before any real-PHI deployment; (4) trace/ledger storage — same MySQL protection domain as the chart, ACL-gated UI, no third-party telemetry (T16); (5) app host — **Railway** (T24), a general-purpose PaaS with no HIPAA BAA for PHI: acceptable this phase because all data is synthetic (OPEN-1), with a BAA-covered host named as a hard gate for real PHI (OPEN-3).
- **Read-only is enforced, not asserted — three layers:** (1) a **module-scoped PHPStan rule** (same custom-rule pattern the repo already uses in `tests/PHPStan/Rules/`) forbids every write API — `sqlInsert`, INSERT/UPDATE/DELETE through any wrapper, service `insert`/`update` methods — outside the whitelisted `mod_copilot_*` repositories, enforced in the module's CI gate beside the repo-diff check (U9); (2) at deploy time, capability reads run on a dedicated **SELECT-only MySQL user**, so even a defect cannot write clinical tables; (3) **LLM egress redaction (decided):** what is *posted to the LLM* is obscured — direct identifiers (name, MRN, DOB, address) are replaced with stable per-session pseudonym tokens before any Vertex call and re-hydrated in the rendered answer after verification. **Ordering (pinned):** redaction wraps *only* the outbound Vertex payload — V1–V6 verify against the real, un-redacted server-side fact set, and re-hydration happens after verification, on render, so the verifier always sees true values, never tokens. The model reasons over clinical values, never over identity; a leaked prompt or completion exposes tokens, not a person. Honest scope: quasi-identifiers (dates, rare lab values) remain, so this is *minimization*, not full de-identification — it is the first concrete piece of OPEN-1's redaction story shipped in v1, and OPEN-1 still gates real PHI.
...
2. **Data-shape surprises** (free-text statuses, units, in-place mutations the contract didn't anticipate) — detected by per-tool failure alert; first move: pull the failing span's payload, add the case to the U2 fixture landmines *before* fixing — every data-quality surprise becomes a permanent eval. Its *silent* variant — a shape that doesn't throw but drops a whole entity (over-stripping) — is caught not by the exception-based per-tool alert but by the **mapping-conservation check (I14)**: `unmapped_n > 0` fires even when nothing errored. Content-level mis-cleaning (a surviving entity read *wrong* rather than dropped) is caught one level deeper by the **cleaning-accuracy audit (§2.6/I15)**.
...
| HIPAA / BAA / PHI | §4; T16; T24; OPEN-1/OPEN-3 |
