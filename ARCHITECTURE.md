...
2. **Data-shape surprises** (free-text statuses, units, in-place mutations the contract didn't anticipate) — detected by per-tool failure alert; first move: pull the failing span's payload, add the case to the U2 fixture landmines *before* fixing — every data-quality surprise becomes a permanent eval. Its *silent* variant — a shape that doesn't throw but drops a whole entity (over-stripping) — is caught not by the exception-based per-tool alert but by the **mapping-conservation check (I14)**: `unmapped_n > 0` fires even when nothing errored.
