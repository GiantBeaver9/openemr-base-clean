...
**Consequence:** the four case-study log questions are answerable from one table by correlation ID alone; alert evaluation (8 alerts incl. the sev-1 V3 trip and the I14 over-stripping alert) rides the existing background-service tick — no new daemon (consistent with T2's rejection of sidecars); the worker-heartbeat alert alone lives on pull paths, since a dead worker can't report itself.
