...
## T18 — LLM platform: Google Gemini on Vertex AI
...
**Why this provider surface:** Vertex is HIPAA-eligible under the GCP BAA — the LLM's legal layer holds wherever the app is hosted (the app runs on **Railway** for cost, T24; Vertex is reached over HTTPS from Railway with a GCP service account). The BAA is the *legal* layer; **egress redaction** (direct identifiers tokenized before any Vertex call, re-hydrated post-verification — ARCHITECTURE.md §4) is the *engineering* layer on top of it, so the compliance story never rests on paper alone. Provider-*enforced* JSON schema materially strengthens V1 (rejection becomes rare instead of retried-often), and context caching is what makes the preloaded-session design (T14) affordable across turns.
**Rejected:**
...
**Chosen:** the deployment scales by resizing the **service** (Railway service CPU/RAM → PHP-FPM worker count; the same stance applied to a GCE machine type before the T24 move to Railway), not by replicating the app. **Why:** OpenEMR is a session-holding monolith and the copilot is in-process by design (T2); each open chat connection holds a worker for the turn's duration, so concurrency is a worker-count problem, and worker count is a resource problem. Horizontal app replicas would immediately buy shared-session storage, sticky connections, and a worker-singleton coordination problem — remodeling the host, which additivity forbids.
**Honest ceiling, named:** the biggest sensible single service serves the v1 target (a clinic, tens of concurrent clinicians) with room; the R8/R9 load tests convert "how many users per core" from guess to number. That ceiling arrives *sooner* on Railway's resource caps than on a large GCE VM (the cost T24 accepts); beyond it (multi-clinic/hospital scale): first migrate to a higher-ceiling host, then read replicas for fact extraction, then extracting the chat loop into its own service — a deliberate re-opening of T2/T24 recorded here in advance, so future-us re-litigates a documented decision instead of discovering an accident.
...
## T24 — Deployment platform: Railway (cost-driven); app-on-Railway / LLM-on-Vertex (user decision of record)

**Context:** the app (OpenEMR + module + MySQL) needs a publicly-reachable deployment (case-study requirement). GCP Compute Engine was the original home (T18/T20's "one cloud" convenience), but it is expensive on the project budget for what is, this phase, a synthetic-data demo.
**Chosen:** host the app on **Railway** — a low-cost container PaaS (git-push deploy, managed MySQL plugin) — while the **LLM stays on Gemini/Vertex** (reached over HTTPS from Railway with a GCP service account). T20's vertical-first stance carries over unchanged: scale the Railway service's CPU/RAM, not app replicas.
**What this trades:**
- *Gained:* materially lower hosting cost at demo / small-clinic scale, and a faster deploy loop.
- *Lost:* T18's "one cloud, one BAA, one IAM" convenience — there are now two vendors (Railway for compute, GCP/Vertex for the LLM). The Vertex BAA for the LLM is **unaffected** — it does not depend on where the app runs.
- *BAA gap, stated honestly:* Railway does **not** offer a HIPAA BAA for PHI (as of this writing — verify before relying on it). This is fine now because the system runs **synthetic patients only** (OPEN-1); it adds a second condition to the real-PHI gate — a BAA-covered host (Railway's enterprise track, or migrate to a cloud VM/k8s) — alongside the redaction/LLM-BAA story OPEN-1 already names. Recorded as OPEN-3.
- *Scaling ceiling, lower than GCE:* Railway suits the demo and a single clinic cheaply; the T20 ceiling arrives sooner on Railway's resource caps, at which point the app migrates to a higher-ceiling host (a deliberate, cost-triggered move).
**Rejected:**
- *Stay on GCP Compute Engine* — cleanest one-cloud story, but hosting cost is exactly what this optimizes; over-provisioning a GCE VM for a synthetic-data demo is the spend being avoided.
- *Move the LLM off Vertex too* — no: Vertex is the HIPAA-eligible LLM surface (T18) and the case study assumes a BAA with the LLM provider; a hosting-cost problem does not touch the LLM decision.
**Reversibility:** the module is additive and platform-agnostic (in-process PHP + MySQL) — changing hosts is a redeploy, not a rewrite. Low-regret, easily reversed.
