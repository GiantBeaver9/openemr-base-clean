...
| `CLINICAL_COPILOT_KNOWLEDGE_EMBED_MODEL` | Vector search (optional) | Gemini embedding model for the pgvector "vector db." Uses the same `CLINICAL_COPILOT_GEMINI_API_KEY`. **Default `text-embedding-004`.** With no key, retrieval runs on full-text search alone. | `CLINICAL_COPILOT_KNOWLEDGE_EMBED_MODEL=text-embedding-004` |
| `CLINICAL_COPILOT_KNOWLEDGE_EMBED_DIM` | Vector search (optional) | Embedding dimensionality. **Default `768`.** MUST match the `vector(N)` column width in `schema.sql` — changing it requires recreating the column. | `CLINICAL_COPILOT_KNOWLEDGE_EMBED_DIM=768` |
